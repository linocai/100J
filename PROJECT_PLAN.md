...
| --- | --- | --- | --- |
| owner-1 | macOS app 每次启动弹"100J 想要使用钥匙串中的机密信息"——ad-hoc 签名每次 cdhash 变化 → Keychain ACL 不认同一个 designated requirement | v1.2.4.1 用"始终允许"workaround，根因未解 | **P0** |
...
- **3 个 deferred 项 + 1 个 owner pain（Keychain 弹窗）**，不夹带其他功能或重构。
...
- 单用户私有云定位不变。
- **新增外部付费依赖**：本期引入 Apple Developer Program（$99/年）以获取 Developer ID Application 证书 + App Group entitlement 的付费签名权限。owner 必须先完成 Apple Developer 注册与证书申请（P0）才能进入 P1（widget 必须付费 team 才能签 App Group entitlement）；同时 P0 顺带修掉 owner-1（Keychain 弹窗）。

### 1.3 预估周期

| Phase | 内容 | 工作量 | 主要驱动 |
| --- | --- | --- | --- |
| P0 | Apple Developer Program 接入 + macOS Developer ID 签名/公证 | S（builder 侧） + 1–2 个工作日 wall clock（Apple 审批） | owner 注册付费 + 申请证书；builder 改打包脚本 |
...
单人节奏 **6–8 个工作日**。其中 P0 的 wall clock 主要来自 Apple Developer Program 审批（1–2 天，owner 等待期可并行启动 P2/P3）。P3 仍是技术复杂度关键路径（复杂度 H），可能再拉长到 8+。P0 是 **P1 的硬前置**（widget 必须付费 team 才能签 App Group），P2/P3 与 P0 完全独立可并行。
...
```
P0 Apple Developer Program 接入 + macOS 签名/公证  (覆盖 owner-1 + 为 P1 铺路)
        │
        └── 硬前置 ──> P1 Widget extension 进 Xcode 工程  (覆盖 #3)

P2 Keyset 分页                                          (覆盖 #14)  ← 与 P0 独立可并行
P3 Alembic 0001 重写                                    (覆盖 #17)  ← 与 P0 独立可并行

   P0 → P1 是唯一硬依赖；P0/P2/P3 三者完全独立，可并行启动。
   在 owner 等 Apple 审批的 1–2 天里，builder 可以推进 P2/P3。
...
- **P0 → P1 是硬依赖**：widget extension 需要在 entitlements 声明 App Group，App Group entitlement 必须由付费 team 的 Apple Distribution / Developer ID 证书签名才被系统接受；ad-hoc 签的 widget 安装即 silently fail。所以 P1 的 Xcode capability 配置必须在 P0 完成（Apple Developer 注册成功 + Team ID 确认）之后进行。
- P2 / P3 与 P0 **完全独立**，可并行。P0 只动 macOS 打包脚本与（可选的）keychain 助手脚本，不动后端，不动 client business code。
- P1 ↔ P2 ↔ P3 之间**没有代码依赖**。各 phase 改动的文件几乎不重合：
  - P0：`frontend/apple/scripts/package-macos-app.sh`、新增 `frontend/apple/scripts/setup-codesign-credentials.sh`、`deployment.md` 末尾 "macOS Release Signing" 段。
...
- P4（发布工程）等所有 phase 都合并后再开。

### 2.3 推荐串行顺序

```
P0 启动（owner 提交 Apple Developer 申请） ┐
                                          ├─ 并行：P2 → P3 推进（builder）
P0 等待 Apple 审批（1–2 天）              ┘
P0 owner 拿到证书 → builder 改打包脚本 → P0 合并
P1（依赖 P0） → P4
```

理由（不是强制）：

1. **P0 owner 申请部分先启动**（成本：填表 + 信用卡）：尽快拿到 Apple 审批排队进度；审批期 owner 无事可做，builder 同时推 P2/P3。
2. **P2 先（与 P0 并行）**：纯后端、无 GUI 操作、风险低，跑一遍快速建立信心。
3. **P3 第二（与 P0 并行）**：纯后端、风险高、需要 owner review。trade off：放在 P2 之后是因为 P3 重写后整套 alembic 链条要重跑回归，与 P2 新增 0007 一起验证更经济。
4. **P0 builder 部分**：Apple 审批通过、owner 拿到证书后，builder 1 天内改完打包脚本 + 文档。
5. **P1 第四**：必须在 P0 完成后开。唯一需要 Xcode GUI 操作的 phase，单独安排 owner 手动窗口。完成后立刻验证 P2/P3 后端没有被 client 改动碰到。
6. **P4 最后**：纯发布工程。
...
**覆盖**：owner-1（macOS 每次启动 Keychain 弹窗，根因 = ad-hoc 签名 cdhash 漂移）+ 为 P1 widget App Group entitlement 铺路（付费 team 才能签）
...
- `frontend/apple/scripts/package-macos-app.sh` 默认 `CODESIGN_IDENTITY=-`（ad-hoc）、`NOTARIZE=0`。每次重打包 cdhash 不同，但 designated requirement 不稳定 → macOS Keychain 在每次启动时弹"100J 想要使用钥匙串中的机密信息"，owner 必须每次点"始终允许"（且不一定记住）。
- 这是**根因**问题：Keychain ACL 通过 designated requirement 判断"这次启动的进程"是否被授权访问已存的 keychain item。Developer ID 签的 app 有稳定的 `anchor apple generic and certificate leaf[subject.CN] = "Developer ID Application: <Name> (<TeamID>)"` requirement，所以同 team 签的任意版本都被 Keychain ACL 接受，不再弹。
- 同时 v1.2.5 P1 widget 要声明 `application-groups` entitlement，**App Group entitlement 必须付费 team 签名**才被系统接受（free / personal team 直接拒）。

owner 已决定付费 $99/年注册 Apple Developer Program，P0 把这一基础设施落地。

打包脚本现状已读（行 1–242）：

- 行 13：`CODESIGN_IDENTITY="${CODESIGN_IDENTITY:-${SIGN_IDENTITY:--}}"` 默认 `-`（ad-hoc）。
- 行 15–19：`NOTARIZE` / `NOTARY_PROFILE` / `APPLE_ID` / `APPLE_TEAM_ID` / `APPLE_APP_SPECIFIC_PASSWORD` 已就位但默认空 / 0。
- 行 38–52：`require_notarization_credentials()` 已实现完整校验。
- 行 82–105：`notarize_app()` 已实现完整 submit → wait → staple → spctl --assess 链。
- 行 235–239：仅当 `NOTARIZE==1` 才走 notarize 路径。

**所以 P0 改动量极小**：把"可选 notarize"变"release 模式强制 notarize"。
...
5. **owner 必须把以下信息抄出来交给 builder**：
   - **Team ID**（很可能仍是 `HX73DFL88G`——free team 已经分配过这个 ID，付费转换后通常保持不变；但**必须人工 verify**，复制到 `PROJECT_PLAN.md §3.0.5` 验收清单中）。
...
改动点（精确到行号，行号基准 = v1.2.4.1 HEAD `c7c067e`）：

1. 在行 20 `STAPLE="${STAPLE:-1}"` 之后追加：
   ```
   RELEASE="${RELEASE:-0}"
   ```
2. 在 `require_notarization_credentials()`（行 38–52）之后追加新函数：
   ```
   require_release_signing() {
     # release 模式 = Developer ID + notarize + staple，三者缺一不可。
     if [[ "$CODESIGN_IDENTITY" == "-" || -z "$CODESIGN_IDENTITY" ]]; then
       echo "RELEASE=1 requires CODESIGN_IDENTITY='Developer ID Application: ...'." >&2
       echo "  Set via env, or source .env.local (P0-3 option A)." >&2
       exit 1
     fi
     if [[ "$CODESIGN_IDENTITY" != "Developer ID Application:"* ]]; then
       echo "RELEASE=1 requires Developer ID Application identity, got: $CODESIGN_IDENTITY" >&2
       echo "  Apple Distribution / Apple Development are not allowed for outside-App-Store macOS distribution." >&2
       exit 1
     fi
     if [[ "$NOTARIZE" != "1" ]]; then
       echo "RELEASE=1 forces NOTARIZE=1. Re-run with NOTARIZE=1 (or omit, RELEASE will set it)." >&2
       exit 1
     fi
     if [[ "$STAPLE" != "1" ]]; then
       echo "RELEASE=1 requires STAPLE=1 (default). Do not override STAPLE=0 in release mode." >&2
       exit 1
     fi
   }
   ```
3. 在脚本顶端的 `if [[ "$NOTARIZE" == "1" ]]; then` 块（行 107–110）**之前**插入：
   ```
   if [[ "$RELEASE" == "1" ]]; then
     NOTARIZE=1
     STAPLE=1
   ...
   ```
   这确保 release 模式自动开启 notarize + staple，并在打包前 fail-fast 校验全部凭据。
4. 在 `codesign_app()`（行 59–80）的 `codesign --verify` 之后追加 release-only assert：
   ```
   if [[ "$RELEASE" == "1" ]]; then
     # release 必须能通过 Gatekeeper 严格检查
     codesign --verify --deep --strict --verbose=2 "$APP_BUNDLE"
     # 校验 Authority 是 Developer ID（不是 ad-hoc / Apple Dev）
     local authority
     authority="$(codesign -dvv "$APP_BUNDLE" 2>&1 | grep -E "^Authority=" | head -1 || true)"
     if [[ "$authority" != "Authority=Developer ID Application:"* ]]; then
       echo "RELEASE assert failed: expected 'Developer ID Application' Authority, got: $authority" >&2
       exit 1
     fi
   fi
   ```
   （注：行 79 已经有一条 `codesign --verify`，release 模式再加一次以确保 fail-fast；接受少量重复）
5. 在 `notarize_app()`（行 82–105）的 `spctl --assess` 之后追加 release-only assert：
   ```
   if [[ "$RELEASE" == "1" ]]; then
     xcrun stapler validate "$APP_BUNDLE"
     # spctl 必须 accepted source=Notarized Developer ID
     local spctl_out
     spctl_out="$(spctl --assess --type execute --verbose=4 "$APP_BUNDLE" 2>&1)"
     if ! grep -q "source=Notarized Developer ID" <<< "$spctl_out"; then
       echo "RELEASE assert failed: spctl did not return 'Notarized Developer ID', got:" >&2
       echo "$spctl_out" >&2
       exit 1
     fi
   fi
...
功能：交互式提示 owner 输入 Apple ID / Team ID / App-Specific Password，调用 `xcrun notarytool store-credentials` 存进 Keychain notarytool profile。
...
PROFILE_NAME="${1:-100j-notary}"

echo "This will store Apple notary credentials in macOS Keychain under profile: $PROFILE_NAME"
echo "You only need to run this once. Subsequent release builds will use the stored profile."
echo

read -r -p "Apple ID email (e.g. linocai13@gmail.com): " APPLE_ID
read -r -p "Team ID (e.g. HX73DFL88G): " APPLE_TEAM_ID
read -r -s -p "App-Specific Password (xxxx-xxxx-xxxx-xxxx, hidden): " APPLE_APP_SPECIFIC_PASSWORD
echo
...
echo
echo "Stored. Test with:"
echo "  RELEASE=1 NOTARY_PROFILE=$PROFILE_NAME frontend/apple/scripts/package-macos-app.sh"
```

**P0-6 `.gitignore` 加 `.env.local`（如未在）** `[builder-ok]`

verify：`grep -E "^\.env\.local$" .gitignore`。如无，追加一行。`.env.local` 文件本身**永远不 commit**。

**P0-7 `deployment.md` 末尾追加 "macOS Release Signing" 段** `[builder-ok]`

新段落（追加到 `deployment.md` 末尾，§ "v1.2.5 Upgrade Notes" 之前或之后，由 P4 时一并 final review）：

```
## macOS Release Signing (v1.2.5+)

Release builds 必须使用 Developer ID Application 证书签名并通过 Apple notarization。
Ad-hoc 签名（`CODESIGN_IDENTITY=-`）仅供本地 dev quick iteration，不得作为 release 发布。

### 前置（owner 一次性）

1. 注册 Apple Developer Program（$99/年）→ 拿到 Team ID（当前：HX73DFL88G）。
2. developer.apple.com → Certificates → 申请 Developer ID Application 证书 → 下载 .cer → 双击导入"登录"钥匙串。
3. appleid.apple.com → 生成 App-Specific Password（名称 100J-notarytool）。
4. 跑 `frontend/apple/scripts/setup-codesign-credentials.sh` 把凭据存进 Keychain notarytool profile（推荐），或写进项目根的 `.env.local`（不入 git）。

### Release build 命令

推荐（Keychain profile 方式）：
```
RELEASE=1 \
...
```
source .env.local
RELEASE=1 frontend/apple/scripts/package-macos-app.sh
```

`RELEASE=1` 会强制开启 NOTARIZE=1 + STAPLE=1，并在打包前 fail-fast 校验：
- CODESIGN_IDENTITY 必须以 "Developer ID Application:" 开头。
- 凭据齐全（NOTARY_PROFILE 或 APPLE_ID + APPLE_TEAM_ID + APPLE_APP_SPECIFIC_PASSWORD）。
- 打包后 `codesign --verify --deep --strict` 通过，Authority 行为 Developer ID。
- `spctl --assess` 返回 `source=Notarized Developer ID`。
- `xcrun stapler validate` 通过。

任何一项 fail 都 exit 1，不出包。

### 一次性迁移（v1.2.4.1 → v1.2.5）

v1.2.4.1 用 ad-hoc 签名，Keychain item `top.linotsai.app.PersonalAffairs.auth` 的 ACL 是 ad-hoc app 写的。
升级到 v1.2.5（Developer ID 签）后**最后弹一次** Keychain 弹窗"100J 想要使用钥匙串中的机密信息"——这是 macOS 看到新签名身份要求 owner 批准 ACL 更新。

**owner 操作**：
- 选项 A（推荐）：弹窗时点"始终允许"，**永久不再弹**。后续所有 Developer ID 签的 v1.2.5+ 版本都被 ACL 接受。
- 选项 B（更干净）：打开 Keychain Access → 登录 → 搜索 `top.linotsai.app.PersonalAffairs.auth` → 整项删除 → 启动 v1.2.5 → 重新登录 → 新 ACL 由 Developer ID app 写入，从此不弹。

升级后第 2 次启动起：**0 弹窗**。如果仍弹，说明签名身份不稳定（不应发生，立即排查）。
```

**P0-8 不动 P1 widget Xcode signing 配置**

P0 完成后 owner 在 Xcode 配置 widget target signing 时，自动用 P0-1 注册成功的付费 team（HX73DFL88G）。这是 P1-1 ~ P1-6 的隐式前置，已在 §3.1 P1 章节描述。P0 不重复改 P1 步骤。
...
| 风险 | 缓解 |
| --- | --- |
| Apple 审批不通过 / 拖延超过 1 周 | owner 用 `linocai13@gmail.com` 此 Apple ID 信誉良好（已有 free team `HX73DFL88G`），审批通常 1–2 天；超时联系 Apple Developer Support；同期 P2/P3 与 P0 独立，不阻塞 builder |
| 付费转换后 Team ID 变了（不再是 `HX73DFL88G`） | owner 必须人工 verify Team ID（P0-1 步 4），如有变化 grep 代码库找所有出现 `HX73DFL88G` 的位置（`grep -rn "HX73DFL88G" frontend/apple/`）批量替换；通报 builder 更新 P1 章节中所有出现的 Team ID |
| App-Specific Password 泄漏（被贴到 git） | 强约定不入 git；选项 B 用 Keychain profile 不出现明文；`.gitignore` 必须含 `.env.local`；commit 前 `git diff` 自查 |
| Apple notarize 服务故障 / 网络阻塞 | notarize 是 Apple 云服务，偶尔慢；脚本用 `--wait` 阻塞等待结果；超时可重跑，幂等 |
| Keychain item ACL 升级时弹窗 owner 误点"拒绝" | deployment.md "一次性迁移" 段明确指引；如点错，按选项 B 删除 keychain item 重新登录恢复 |
| 旧 ad-hoc 签的 v1.2.4.1 包还在 owner 机器上 → 与 v1.2.5 Developer ID 包混装 → ACL 混乱 | 安装 v1.2.5 前 owner 先 `rm -rf /Applications/100J.app`，再装新包；deployment.md 强调"清装" |
| `codesign` 默认 Authority 字符串与 grep 模式不匹配（如 macOS 版本差异） | 在 builder 本机测过的 `codesign -dvv` 输出格式：`Authority=Developer ID Application: <Name> (HX73DFL88G)`；如未来格式变，更新 assert grep 模式 |
...
| 主题 | 涉及文件 | 涉及 phase |
| --- | --- | --- |
| macOS release signing 脚本 | `frontend/apple/scripts/package-macos-app.sh`（改）、`frontend/apple/scripts/setup-codesign-credentials.sh`（新建）、`.gitignore`（如缺 `.env.local`）、`deployment.md`（"macOS Release Signing" 段） | P0, P4 |
...
- [ ] **P0 macOS release 签名链**（owner 在本机一次性跑通）：
  ...
  - [ ] `xcrun stapler validate dist/100J.app` exit 0
- [ ] **P0 owner-1 验收**：干净 macOS 账户（或先删 `top.linotsai.app.PersonalAffairs.auth` keychain item）装 v1.2.5 → 登录 → 重启 app 3 次 → 0 次 Keychain 弹窗
...
- P2 / P3 不动客户端代码；P0 仅动打包脚本，P1 仅动 widget extension 与 pbxproj。
...
1. 开新分支 `v1.2.5-dev` from `main`（HEAD `c7c067e`，tag `v1.2.4.1`）。
2. **P0 owner 部分 day-0 立即启动**：owner 当天提交 Apple Developer Program 注册（信用卡支付 $99），开启 1–2 天审批等待窗口。builder 不阻塞，进入步骤 3/4。
3. **P2 先开**（与 P0 owner 等待并行）：纯后端、低风险、快速建信心。每个 commit 聚焦单个 endpoint 接入新 paginate，最后一个 commit 加 0007 migration + model `__table_args__`。
4. **P3 第二**（与 P0 owner 等待并行）：建议拆 PR：
   - PR1：P3-1 推断 + P3-2 重写 0001 + P3-3 baseline check。
   - PR2：P3-4 drift guard 增强 + hash 文件。
   - owner 必须在 PR1 merge 前本地复现 P3-5 三路径 diff。
5. **P0 builder 部分**：owner Apple 审批通过 + 拿到证书 + 完成 setup-codesign-credentials.sh 一次性配置后，builder 1 天内提交 P0 PR（package-macos-app.sh 改动 + setup-codesign-credentials.sh 新建 + .gitignore + deployment.md 段落）。owner 在本机跑通完整 `RELEASE=1` 流程并清单 §6.3 P0 全过后 merge。
6. **P1 第四**（P0 merge 后开）：owner 在 Xcode GUI 做 P1-1 ~ P1-6（一次完成，提一个 commit 含 pbxproj + entitlements + Info.plist）；builder 后续提 P1-7 测试 + P1-8 文档。
7. **P4 最后**：单独一个 PR，含版本号 + OpenAPI snapshot + deployment.md final review（含 P0 macOS Release Signing 段确认）+ verify-release.sh + prod-check.sh 增项。
8. 跑 §6.1 / §6.2 / §6.3 全部清单，全绿后 `git tag v1.2.5` → 合 main → push。
9. App Store Connect 上传 archive → TestFlight beta → 灰度验证 → 推送至全部用户（仅 owner 一人）。
...
- owner 决定付费注册 Apple Developer Program（$99/年），引入付费签名能力。
- 新增 Phase **P0**（在 P1 之前）：覆盖 owner-1（macOS 启动 Keychain 弹窗，根因 = ad-hoc 签名 cdhash 漂移）+ 为 P1 widget App Group entitlement 铺路。
- §1.1 目标表加 owner-1 一行；§1.2 注明引入 Apple Developer 付费依赖；§1.3 总周期 5–7 → **6–8 个工作日**（含 Apple 审批 1–2 天 wall clock，但与 P2/P3 builder 推进并行）；§1.4 Release Criteria 加 2 条（release signing 链全通 + 真机 0 Keychain 弹窗）。
- §2 依赖图：**P0 → P1 是唯一硬依赖**；P0/P2/P3 三者完全独立可并行。
- §3 Phase 细则：在 P1 之前插入完整 P0 章节（背景 / 8 步改动清单 / 测试计划 / 验收 / 风险）。P0 改动文件：`frontend/apple/scripts/package-macos-app.sh`（加 `RELEASE=1` 强制路径）、新建 `frontend/apple/scripts/setup-codesign-credentials.sh`、`.gitignore`、`deployment.md` "macOS Release Signing" 段。
- §6.1 / §6.3 加 P0 验收项；§7.2 加 "Developer ID → ad-hoc 回退" 段；§7.3.1 加 "一次性迁移说明"。
- §8 builder 执行顺序：P0 owner 部分 day-0 启动，与 P2/P3 并行；P0 builder 部分在 owner 拿到证书后 1 天完成；P1 等 P0 merge。
- P1/P2/P3/P4 phase 编号与内部细节**未变**（仅 P1 章节加了 "前置依赖：P0 必须先完成" 一行说明）。
