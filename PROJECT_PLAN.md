...
### [2026-05-27] P1 builder 实施偏离：保留 CaptureParser + AgentNaturalCommandBuilder
- 变更内容：删除 UniversalComposerViewModel.swift / ComposerSheet.swift 及 App 层全部 Composer 引用；**但保留** CaptureParser.swift + AgentNaturalCommandBuilder（位于 `PersonalAffairsCore/`，被 AgentReviewSession.compose 调用）。
- 变更原因：plan §3.1.2 / §3.1.6 / owner prompt 第 5 条均明确"Agent 屏所有功能保留"。AgentScreen 的命令输入区（`commandInput`）→ `composeAgentCommand()` → `AgentViewModel.composePendingInput()` → `AgentReviewSession.compose(text:)` 直接调 `CaptureParser.parse` + `AgentNaturalCommandBuilder.build`。这两个类型与 Agent 屏自然语言解析路径绑死，删了 Agent 屏自然语言输入立刻断。
- 与 plan 不一致：plan §3.1.5 静态校验列了 "`grep CaptureParser frontend/apple/Sources/` 必须 0 命中"，与 §3.1.2 保留 Agent 屏功能矛盾。按 owner prompt "AgentScreen 自身输入区域不能删" 优先。最终 grep CaptureParser 在 Sources/ 仍有 2 处真实代码命中（CaptureParser.swift 本身 + AgentReviewSession.swift），但 `Sources/PersonalAffairsApp/` App 层 0 真实代码命中（注释引用 3 处）。
- 影响范围：Phase P1（仅 builder 路径，无后端 / 测试基线影响）。
- AppShortcuts 处理：`CreateTaskIntent` 重写直接调 TaskRepository.create + 简单 公司/工作 关键词推断 scope；`AskAgentIntent` 删除（依赖 CaptureParser 走 Agent execute 不属于"Agent 屏"，且 plan 没明确要求保留 Siri intent 链）。
- 测试净变化：Swift 53 → 63（+10 = InlineQuickAddRowTests 4 条 + AppModelInlineCreateTests 6 条；无 Composer 相关测试删除，因为旧测试集中本无 UniversalComposerViewModelTests / ComposerSheetTests 文件）。
